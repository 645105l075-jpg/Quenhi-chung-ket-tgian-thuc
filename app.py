...
def ors_routing(coords, base_url=DEFAULT_OSRM_BASE_URL, batch_size=4, timeout=60):
    ...
    Mỗi request chỉ chứa tối đa batch_size origins x batch_size destinations.
    Với batch_size=4, URL chỉ chứa tối đa 8 tọa độ duy nhất (thường ít hơn
    nếu origin và destination trùng nhau), thay vì 4 x toàn bộ 100 điểm.
    Vẫn dùng khoảng cách/thời gian đường bộ thực tế từ OSRM/OSM.
    """
                ...
                try:
                    ...
                    if response.status_code == 414:
                        raise OSRMError(
                            ...
                            f"destination {di + 1}-{dest_idx[-1] + 1}. "
                            "Block hiện tại đã là 4×4; không cần giảm thêm."
                        )
