...
def get_osrm_matrices_safe(coords, base_url=DEFAULT_OSRM_BASE_URL, batch_size=8, timeout=60):
    """
    Lấy ma trận OSRM theo từng batch origin để tránh HTTP 414 khi có nhiều điểm.
    Vẫn dùng mạng đường thực tế của OSRM/OSM; KHÔNG dùng Haversine.

    coords: iterable[(latitude, longitude)]
    batch_size: số điểm origin gửi mỗi request. Destination vẫn là toàn bộ điểm.
    """
    ...
    for start in range(0, n, batch_size):
        ...
        origins = coords[start:end]

        # Chỉ đưa batch origins + toàn bộ destinations vào URL.
        # Với 100 điểm, batch 8 giữ URL ngắn và tránh HTTP 414.
        all_points = origins + coords
        coord_string = ";".join(
            f"{lon:.6f},{lat:.6f}" for lat, lon in all_points
        )

        sources = ";".join(str(i) for i in range(len(origins)))
        destinations = ";".join(
            str(len(origins) + j) for j in range(n)
        )
        ...
        for attempt in range(3):
            try:
                ...
                if response.status_code == 414:
                    raise OSRMError(
                        f"OSRM vẫn trả HTTP 414 ở batch {start + 1}-{end}. "
                        "Hãy giảm batch_size xuống 4."
                    )
                if response.status_code != 200:
                    raise OSRMError(
                        f"OSRM HTTP {response.status_code}: {response.text[:250]}"
                    )

                data = response.json()
                if data.get("code") != "Ok":
                    raise OSRMError(
                        f"OSRM trả về lỗi: {data.get('code', 'Unknown')}"
                    )
                ...
                for local_i, global_i in enumerate(range(start, end)):
                    for j in range(n):
                        if ds[local_i][j] is None or ts[local_i][j] is None:
                            raise OSRMError(
                                f"OSRM không tìm được đường từ điểm {global_i} đến điểm {j}."
                            )
                        distance[global_i, j] = float(ds[local_i][j])
                        duration[global_i, j] = float(ts[local_i][j])
                break
    ...
    if np.isnan(distance).any() or np.isnan(duration).any():
        raise OSRMError("Ma trận OSRM còn ô trống sau khi chia batch.")
...
if run_btn:
    ...
    with st.spinner("Đang gọi OSRM để lấy ma trận khoảng cách/thời gian đường bộ..."):
        try:
            matrix_result = get_osrm_matrices_safe(
                coords,
...
if run_compare_btn:
    ...
    with st.spinner("Đang lấy ma trận khoảng cách/thời gian từ OSRM cho bộ 100 điểm..."):
        try:
            cmp_matrix = get_osrm_matrices_safe(
                cmp_coords,
                base_url=osrm_base_url,
                batch_size=8,
            )
