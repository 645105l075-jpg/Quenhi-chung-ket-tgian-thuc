...
from osrm_public_routing import (
    DEFAULT_OSRM_BASE_URL,
    OSRMError,
    check_point_count_limit,
    get_osrm_matrices,
    get_osrm_route_geometry,
    get_osrm_matrices_safe,
    find_nearest_road_segment,
    get_osrm_avoiding_route,
)
...
def _cached_get_osrm_matrices(coords_tuple, base_url, batch_size):
    """Bọc cache quanh get_osrm_matrices_safe: cùng 1 bộ điểm + cùng
    base_url + cùng batch_size sẽ KHÔNG gọi lại ORS/OSRM lần 2 trong vòng
    1 giờ (ttl=3600s), kể cả khi Streamlit tự rerun script do bạn tương tác
    UI. Đây là nguyên nhân phổ biến khiến quota ORS bị dùng hết rất nhanh
    dù bạn không cố ý gọi nhiều lần.
    coords_tuple PHẢI là tuple (không phải list) để Streamlit hash được.
    """
    return get_osrm_matrices_safe(
        list(coords_tuple), base_url=base_url, batch_size=batch_size
    )
...
if run_btn:
    ...
    with st.spinner("Đang gọi OSRM public server để lấy ma trận khoảng cách/thời gian đường bộ..."):
        try:
            matrix_result = _cached_get_osrm_matrices(
                tuple(tuple(c) for c in coords),
                osrm_base_url,
                4,  # OSRM public server dùng GET -> giữ batch nhỏ để tránh HTTP 414
            )
...
if run_compare_btn:
    ...
    cmp_matrix = None
    with st.spinner("Đang lấy ma trận khoảng cách/thời gian từ OSRM public server cho bộ 100 điểm..."):
        try:
            cmp_matrix = _cached_get_osrm_matrices(
                tuple(tuple(c) for c in cmp_coords),
                osrm_base_url,
                4,  # OSRM public server dùng GET -> giữ batch nhỏ để tránh HTTP 414
            )
        except OSRMError as exc:
...
INCIDENT_EDGE_PENALTY_MULTIPLIER = 50.0  # nhân chi phí lên gấp nhiều lần thay
# vì đặt vô cực - tránh OR-Tools coi bài toán là infeasible khi đây là cạnh
# BẮT BUỘC duy nhất (VD chỉ có 1 đường vào 1 ngõ cụt) - vẫn ưu tiên rất mạnh
# việc tránh cạnh này nếu còn đường khác.


def _apply_incident_penalty(sub_indices, sub_dist, sub_dur, blocked_edges):
    """Sửa TẠI CHỖ sub_dist/sub_dur: với mọi cặp (i, j) trong sub_indices mà
    cạnh logic (global) của chúng nằm trong blocked_edges, nhân chi phí
    khoảng cách/thời gian lên rất lớn để OR-Tools tránh đi thẳng qua cạnh đó
    khi xếp lại thứ tự các điểm ghé thăm (mục 4/11 của yêu cầu: gán chi phí
    cực lớn thay vì loại bỏ hẳn, để bài toán không bị infeasible)."""
    ...
    for i in range(n):
        for j in range(n):
            if i == j:
                continue
            if frozenset((sub_indices[i], sub_indices[j])) in blocked_edges:
                sub_dist[i][j] = sub_dist[i][j] * INCIDENT_EDGE_PENALTY_MULTIPLIER + 1_000_000
                sub_dur[i][j] = sub_dur[i][j] * INCIDENT_EDGE_PENALTY_MULTIPLIER + 100_000


def _fleet_reoptimize_nodes(node_indices, base_url, capacity_kg, blocked_edges=None):
    ...
    blocked_edges: set[frozenset] các cạnh (node_a, node_b) hiện đang có sự
    cố - nếu truyền vào, chi phí các cạnh này bị phạt nặng để OR-Tools ưu
    tiên KHÔNG xếp 2 điểm đó kế tiếp nhau trong tuyến mới."""
    sub_indices = [depot_index] + list(node_indices)
    sub_dist = [[dist_m_full[a][b] for b in sub_indices] for a in sub_indices]
    sub_dur = [[dur_s_full[a][b] for b in sub_indices] for a in sub_indices]
    _apply_incident_penalty(sub_indices, sub_dist, sub_dur, blocked_edges)
    sub_demands = [demands_full[i] for i in sub_indices]
    sub_service = [service_times_s_full[i] for i in sub_indices]
    cfg = OptimizeConfig(
...
INCIDENT_TYPES = {
    "🚧 Đường bị chặn": "blocked",
    "🚗 Tai nạn": "accident",
    "🚦 Ùn tắc nghiêm trọng": "congestion",
}


def _collect_edge_geometries(fsim: FleetSimulator) -> dict:
    """Gom road geometry OSRM thật của TẤT CẢ leg (đã đi qua + sắp đi) của
    mọi xe đang hoạt động - đây là tập 'segment' mà người dùng có thể click
    trúng trên bản đồ (Yêu cầu #11: dùng road network thật, không suy diễn
    đường thẳng giữa 2 toạ độ node)."""
    edges = {}
    for t in fsim.trucks.values():
        for leg in t.legs:
            if leg.geometry:
                edges[(leg.from_node, leg.to_node)] = leg.geometry
    return edges


def _add_incident(fsim: FleetSimulator, incident_type_label: str, lat: float, lon: float, node_a: int, node_b: int) -> dict:
    """add_incident(): tạo 1 sự cố mới + block cạnh tương ứng trong
    fsim.blocked_edges (state DUY NHẤT cho 'cạnh nào đang bị chặn' - tái sử
    dụng thay vì tạo thêm blocked_segments riêng)."""
    incident = {
        "id": uuid.uuid4().hex[:8],
        "type_label": incident_type_label,
        "lat": lat, "lon": lon,
        "node_a": node_a, "node_b": node_b,
        "created_min": fsim.clock_min,
    }
    st.session_state.incidents.append(incident)
    fsim.block_road(node_a, node_b)
    return incident


def _remove_incident(fsim: FleetSimulator, incident_id: str) -> None:
    """remove_incident(): gỡ 1 sự cố cụ thể (theo id) và unblock cạnh liên
    quan. Không hoàn tác geometry đã detour của các xe (an toàn hơn là bắt
    xe đổi hướng đột ngột lần nữa); tuyến sẽ chỉ dùng lại đường ngắn nhất ở
    lần tái tối ưu tiếp theo."""
    incident = next((i for i in st.session_state.incidents if i["id"] == incident_id), None)
    if incident is None:
        return
    fsim.unblock_road(incident["node_a"], incident["node_b"])
    st.session_state.incidents = [i for i in st.session_state.incidents if i["id"] != incident_id]


def _clear_all_incidents(fsim: FleetSimulator) -> None:
    for incident in list(st.session_state.incidents):
        fsim.unblock_road(incident["node_a"], incident["node_b"])
    st.session_state.incidents = []
    st.session_state.incident_impact = None


def find_nearest_road_segment_or_none(fsim: FleetSimulator, click_lat: float, click_lon: float):
    """find_nearest_road_segment(): wrapper mỏng quanh hàm cùng tên trong
    routing.py, tự gom edge_geometries từ fsim hiện tại."""
    edge_geometries = _collect_edge_geometries(fsim)
    if not edge_geometries:
        return None
    return find_nearest_road_segment(click_lat, click_lon, edge_geometries)


def reroute_affected_vehicles(fsim: FleetSimulator, node_a: int, node_b: int, incident: dict, base_url: str) -> dict:
    """reroute_affected_vehicles() / update_routes_after_incident(): đây là
    bước 'Xác định road segment' (đã làm ở caller) -> 'Block segment' (đã
    làm ở _add_incident) -> hàm này lo phần còn lại của luồng:
    Cập nhật network/travel cost -> Re-route -> Tuyến mới -> Xe tiếp tục.

    Với TỪNG xe bị ảnh hưởng (fsim.find_affected_trucks): gọi
    get_osrm_avoiding_route() để lấy 1 tuyến OSRM thật đi vòng qua sự cố cho
    ĐÚNG leg bị chặn, rồi fsim.set_leg_geometry() để xe tiếp tục di chuyển
    dọc geometry mới - KHÔNG đổi thứ tự các điểm còn lại trên tuyến (capacity,
    time window, điểm chưa phục vụ giữ nguyên vì đây chỉ là đổi ĐƯỜNG ĐI giữa
    2 điểm đã quyết định, không đổi LỊCH TRÌNH). Xe không bị ảnh hưởng
    (fsim.find_affected_trucks không trả về) giữ nguyên tuyến, không động tới.
    """
    affected = fsim.find_affected_trucks(node_a, node_b)
    result = {
        "incident": incident,
        "affected_vehicle_ids": list(affected.keys()),
        "before_total_km": 0.0,
        "after_total_km": 0.0,
        "num_points_rerouted": 0,
        "failed_vehicle_ids": [],
    ...
    for vid, leg_idx in affected.items():
        truck = fsim.trucks[vid]
        before_m = fsim.remaining_route_length_m(vid)
        result["before_total_km"] += before_m / 1000.0

        leg = truck.legs[leg_idx]
        # Nếu đây CHÍNH LÀ leg xe đang đi dở, xuất phát từ vị trí GPS hiện
        # tại của xe (không quay lại từ đầu leg) - đúng yêu cầu #4 "xe tiếp
        # tục từ vị trí hiện tại". Nếu là leg TƯƠNG LAI (chưa tới), xuất phát
        # từ node đầu leg như bình thường.
        start_coord = truck.position if (leg_idx == truck.leg_idx and truck.position) else coords[leg.from_node]
        end_coord = coords[leg.to_node]

        new_geom = get_osrm_avoiding_route(
            start_coord, end_coord, incident["lat"], incident["lon"], base_url=base_url,
        )
        if new_geom:
            fsim.set_leg_geometry(vid, leg_idx, new_geom)
            after_m = fsim.remaining_route_length_m(vid)
            result["after_total_km"] += after_m / 1000.0
            result["num_points_rerouted"] += len(truck.pending_nodes)
            fsim.log(f"✓ Vehicle {vid}: đã tìm thấy tuyến thay thế né sự cố tại {leg.from_node}↔{leg.to_node}.")
        else:
            result["failed_vehicle_ids"].append(vid)
            result["after_total_km"] += before_m / 1000.0  # không đổi được -> xe giữ nguyên trạng thái BLOCKED an toàn
            fsim.log(f"❌ Vehicle {vid}: không tìm được tuyến thay thế, xe tạm dừng chờ gỡ sự cố hoặc tái tối ưu thủ công.")

    return result


def _reroute_after_incident(fsim: FleetSimulator, node_a: int, node_b: int, incident: dict, base_url: str) -> dict:
    with st.spinner("⚠️ Phát hiện sự cố! Đang tái tối ưu tuyến..."):
        result = reroute_affected_vehicles(fsim, node_a, node_b, incident, base_url)
    return result
...
if st.session_state.get("fleet_sim_enabled") and "fleet_sim" in st.session_state:
    ...
    # ---- State cho mô phỏng sự cố (mục 9: tái sử dụng state hiện có -
    # fsim.blocked_edges, fsim.trucks, fsim.clock_min - KHÔNG tạo hệ thống
    # state thứ hai; chỉ bổ sung "incidents" để lưu metadata hiển thị mà
    # blocked_edges (chỉ là set cạnh) không lưu được: toạ độ click, loại sự
    # cố, thời điểm tạo, id để xoá) ----
    st.session_state.setdefault("incidents", [])
    st.session_state.setdefault("incident_mode_enabled", False)
    st.session_state.setdefault("incident_impact", None)
    ...
    with st.expander("📈 Rác thực tế vượt dự báo", expanded=False):
    ...
    with st.expander("🚧 Mô phỏng sự cố", expanded=st.session_state.incident_mode_enabled):
        st.checkbox(
            "Bật chế độ tạo sự cố (click trực tiếp lên bản đồ bên dưới để tạo)",
            key="incident_mode_enabled",
        )
        incident_type_label = st.selectbox(
            "Loại sự cố", list(INCIDENT_TYPES.keys()), key="incident_type_label",
        )
        st.caption(
            "MVP: cả 3 loại hiện đều được xử lý như 'chặn cứng' đoạn đường (chi phí ≈ vô cực). "
            "Phân biệt mức độ (VD ùn tắc chỉ tăng thời gian di chuyển thay vì chặn hẳn) là hướng "
            "mở rộng tiếp theo, không đổi kiến trúc hiện tại."
        )

        st.markdown("**Hoặc tạo thủ công theo cặp điểm (khi không tiện click bản đồ):**")
        ...
        if st.button("🚧 Tạo sự cố A↔B"):
            a_idx, b_idx = index_of_node[node_a], index_of_node[node_b]
            mid_lat = (coords[a_idx][0] + coords[b_idx][0]) / 2.0
            mid_lon = (coords[a_idx][1] + coords[b_idx][1]) / 2.0
            incident = _add_incident(fsim, incident_type_label, mid_lat, mid_lon, a_idx, b_idx)
            st.session_state.incident_impact = _reroute_after_incident(
                fsim, a_idx, b_idx, incident, res["osrm_base_url"]
            )
            st.rerun()

        st.divider()
        active_incidents = st.session_state.incidents
        st.markdown(f"**Sự cố đang hoạt động: {len(active_incidents)}**")
        if active_incidents:
            for incident in active_incidents:
                label_a = node_ids_full[incident["node_a"]] if incident["node_a"] < len(node_ids_full) else incident["node_a"]
                label_b = node_ids_full[incident["node_b"]] if incident["node_b"] < len(node_ids_full) else incident["node_b"]
                rowc1, rowc2 = st.columns([4, 1])
                with rowc1:
                    st.write(f"{incident['type_label']} – Segment: {label_a} ↔ {label_b}")
                with rowc2:
                    if st.button("Xóa", key=f"remove_incident_{incident['id']}"):
                        _remove_incident(fsim, incident["id"])
                        st.rerun()
            if st.button("🗑️ Xóa tất cả sự cố"):
                _clear_all_incidents(fsim)
                st.rerun()
        else:
            st.caption("Chưa có sự cố nào.")

    # ---- Thông báo kết quả re-route của sự cố GẦN NHẤT (mục 6/12) ----
    impact = st.session_state.get("incident_impact")
    if impact:
        if impact["not_affected"]:
            st.success("✓ Sự cố không ảnh hưởng đến các tuyến hiện tại.")
        else:
            if impact["failed_vehicle_ids"]:
                for vid in impact["failed_vehicle_ids"]:
                    st.error(f"❌ Không tìm được tuyến thay thế cho Vehicle {vid}.")
            ok_vehicles = [v for v in impact["affected_vehicle_ids"] if v not in impact["failed_vehicle_ids"]]
            if ok_vehicles:
                st.success(f"✓ Đã tìm thấy tuyến thay thế cho Vehicle {', '.join(str(v) for v in ok_vehicles)}.")
    ...
    if st.session_state.get("fleet_sim_running", True):
        ...
        for t in list(fsim.trucks.values()):
            if t.status == STATE_FULL and t.needs_reopt:
                ...
                seq, msg = _fleet_reoptimize_nodes(
                    t.pending_nodes, res["osrm_base_url"], t.capacity_kg, blocked_edges=fsim.blocked_edges
                )
                if seq:
        ...
        if pool and idle:
            seq, msg = _fleet_reoptimize_nodes(
                pool, res["osrm_base_url"], vehicle_capacity_kg, blocked_edges=fsim.blocked_edges
            )
            if seq:
                legs = build_route_legs(seq, geometry_fetch)
    ...
    # ---- Bảng trước/sau sự cố (mục 7) ----
    if impact and not impact["not_affected"]:
        before_km = impact["before_total_km"]
        after_km = impact["after_total_km"]
        impact_df = pd.DataFrame([
            {
                "Chỉ tiêu": "Quãng đường còn lại (các xe bị ảnh hưởng)",
                "Trước sự cố": f"{before_km:.2f} km", "Sau sự cố": f"{after_km:.2f} km",
            },
            {
                "Chỉ tiêu": "Số xe bị ảnh hưởng", "Trước sự cố": "-",
                "Sau sự cố": str(len(impact["affected_vehicle_ids"])),
            },
            {
                "Chỉ tiêu": "Số điểm phải re-route", "Trước sự cố": "-",
                "Sau sự cố": str(impact["num_points_rerouted"]),
            },
        ])
        st.markdown("**📊 Trước / sau sự cố**")
        st.dataframe(impact_df, use_container_width=True, hide_index=True)
        delta_km = after_km - before_km
        avg_speed = max(1.0, float(fleet_speed_kmh))
        delta_min = (delta_km / avg_speed) * 60.0
        st.caption(f"Chi phí phát sinh: {delta_km:+.2f} km / {delta_min:+.1f} phút (ước tính theo tốc độ mô phỏng {avg_speed:.0f} km/h).")
    ...
    for incident in st.session_state.incidents:
        folium.Marker(
            ...
            popup=f"{incident['type_label']} — {incident['node_a']}↔{incident['node_b']}",
    ...
    click_mode = st.session_state.incident_mode_enabled
    map_data = st_folium(
        fleet_map, use_container_width=True, height=550,
        returned_objects=["last_clicked"] if click_mode else [],
        key="fleet_sim_map",
    )
    if click_mode:
        st.caption("🖱️ Chế độ tạo sự cố đang BẬT: click vào 1 điểm gần đoạn đường trên bản đồ để tạo sự cố.")

    # ---- Click → Xác định segment gần nhất → Block → Re-route → Continue ----
    if click_mode and map_data and map_data.get("last_clicked"):
        ...
        if click_key != st.session_state.last_processed_click:
            ...
            nearest = find_nearest_road_segment_or_none(fsim, clicked["lat"], clicked["lng"])
            if nearest is None:
                st.warning(
                    "Vị trí click quá xa mọi tuyến đường hiện tại (>120m) — không xác định được segment. "
                    "Hãy click sát vào 1 đoạn đường (đường nét màu) đang hiển thị trên bản đồ."
                )
            else:
                a_idx, b_idx, _dist = nearest
                incident = _add_incident(
                    fsim, st.session_state.incident_type_label, clicked["lat"], clicked["lng"], a_idx, b_idx,
                )
                st.session_state.incident_impact = _reroute_after_incident(
                    fsim, a_idx, b_idx, incident, res["osrm_base_url"]
                )
